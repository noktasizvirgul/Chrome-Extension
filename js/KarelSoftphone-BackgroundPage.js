var pref = KarelSoftphone_Preferences; // alias for the KarelSoftphone_Preferences object
...
function callSetupSuccess(data, status, xhr) {
	...
	if (xhr.status != 200) {
		if (xhr.status == 404) {
			msg = 'KarelSoftphone Request Error: server returned a 404 (Not Found) error.';
		}
		else {
			msg = 'KarelSoftphone Network Error: Are you disconnected from the network?';
		}
